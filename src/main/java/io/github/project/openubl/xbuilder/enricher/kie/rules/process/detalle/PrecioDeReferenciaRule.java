...
public class PrecioDeReferenciaRule extends AbstractRule {

    @Override
    public boolean test(Object object) {
        return isBaseDocumentoDetalle.test(object) && whenBaseDocumentoDetalle.apply(object)
                .map(documento -> documento.getPrecioReferencia() == null && documento.getPrecio() != null && documento.getIgvTipo() != null)
                .orElse(false);
    }
    ...
    public void modify(Object object) {
        Consumer<DocumentoDetalle> consumer = detalle -> {
            ...
            if (detalle.isPrecioConImpuestos()) {
                precioReferencia = catalog7.isOperacionOnerosa() ?
                        detalle.getPrecio().divide(detalle.getIgvTasa().add(BigDecimal.ONE), 10, RoundingMode.HALF_EVEN) :
                        detalle.getPrecio();
            } else {
                precioReferencia = catalog7.isOperacionOnerosa() ?
                        detalle.getPrecio().multiply(detalle.getIgvTasa().add(BigDecimal.ONE)) :
                        detalle.getPrecio();
            }
