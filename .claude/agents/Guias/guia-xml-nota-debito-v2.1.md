...
La Nota de Debito Electronica es un documento que se emite para incrementar (aumentar) el valor de una Factura o Boleta emitida con anterioridad. Se utiliza para:
- Aumento en el valor de la operacion
- Penalidades
- Intereses por mora
- Otros cargos

Se basa en el estandar UBL 2.1 usando el elemento raiz `<DebitNote>`.

### 1.2 Condiciones

- Solo se puede emitir una Nota de Debito para modificar un comprobante de pago que haya sido previamente enviado a SUNAT.
- La Nota de Debito debe referenciar el documento original.
...
- Para Notas de Debito asociadas a Facturas: Serie inicia con "F" (ej. FF01, FD01)
- Para Notas de Debito asociadas a Boletas: Serie inicia con "B" (ej. BC01, BD01)
...
| `cbc:ResponseCode` | Tipo de nota de debito (Catalogo No. 10) | an | Si |
| `cbc:Description` | Motivo o sustento | an | Si |

Catalogo No. 10 - Codigos de Tipo de Nota de Debito:
| Codigo | Descripcion |
|--------|------------|
...
<cac:DiscrepancyResponse>
  ...
  <cbc:ResponseCode listAgencyName="PE:SUNAT"
    ...
    listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo10">01</cbc:ResponseCode>
  <cbc:Description>Intereses por mora</cbc:Description>
</cac:DiscrepancyResponse>
```
...
Same structure as Invoice/CreditNote. Global tax totals with TaxSubtotal for each tax type.
...
## 11. Totales Monetarios (RequestedMonetaryTotal)

**IMPORTANTE:** A diferencia de Invoice y CreditNote que usan `LegalMonetaryTotal`, la Nota de Debito usa `RequestedMonetaryTotal`.
...
## 12. Detalle de Items (DebitNoteLine)

**IMPORTANTE:** El elemento de linea se llama `DebitNoteLine` (no `InvoiceLine` ni `CreditNoteLine`).
...
| `cbc:DebitedQuantity` | Cantidad (nota: usa `DebitedQuantity`) | Si |
| `cbc:DebitedQuantity/@unitCode` | Unidad de medida (Catalogo No. 03) | Si |
| `cbc:LineExtensionAmount` | Valor de venta del item | Si |

```xml
<cac:DebitNoteLine>
  <cbc:ID>1</cbc:ID>
  <cbc:DebitedQuantity unitCode="NIU" unitCodeListID="UN/ECE rec 20"
    unitCodeListAgencyName="United Nations Economic Commission for Europe">10</cbc:DebitedQuantity>
  <cbc:LineExtensionAmount currencyID="PEN">1000.00</cbc:LineExtensionAmount>
```
...
| 11-16 | Gravado - Retiros | IGV (1000) |
...
```xml
<cac:Item>
  <cbc:Description><![CDATA[Intereses por mora en pago]]></cbc:Description>
</cac:Item>
```
...
### 13.1 Intereses por Mora (Tipo 01)

Se emite cuando el adquiriente no ha cumplido con el pago en la fecha pactada y se aplican intereses.

### 13.2 Aumento en el Valor (Tipo 02)

Se emite cuando se incrementa el valor de la operacion original por ajustes o modificaciones acordadas.

### 13.3 Penalidades / Otros Conceptos (Tipo 03)

Se emite por penalidades contractuales u otros conceptos que incrementan el monto a cobrar.

### 13.4 Ajustes de Exportacion (Tipo 10)

Para ajustes en operaciones de exportacion que incrementan el valor.

### 13.5 Ajustes IVAP (Tipo 11)

Para ajustes en operaciones sujetas al IVAP que incrementan el valor.

---

## 14. Diferencias Clave con Invoice y CreditNote

| Aspecto | Invoice | CreditNote | DebitNote |
|---------|---------|------------|-----------|
| Elemento raiz | `<Invoice>` | `<CreditNote>` | `<DebitNote>` |
| Tipo de documento | `01`/`03` | `07` | `08` |
| Elemento de linea | `InvoiceLine` | `CreditNoteLine` | `DebitNoteLine` |
| Cantidad | `InvoicedQuantity` | `CreditedQuantity` | `DebitedQuantity` |
| Totales monetarios | `LegalMonetaryTotal` | `LegalMonetaryTotal` | `RequestedMonetaryTotal` |
| Ref doc original | No aplica | `DiscrepancyResponse` + `BillingReference` | `DiscrepancyResponse` + `BillingReference` |
| Catalogo motivo | No aplica | Catalogo No. 09 | Catalogo No. 10 |
| Efecto | Documento original | Disminuye valor | Aumenta valor |
...
## 16. Ejemplo Completo - Nota de Debito por Intereses por Mora
...
<DebitNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:DebitNote-2"
  ...
  <cac:DebitNoteLine>
    <cbc:ID>1</cbc:ID>
    <cbc:DebitedQuantity unitCode="ZZ" unitCodeListID="UN/ECE rec 20"
      unitCodeListAgencyName="United Nations Economic Commission for Europe">1</cbc:DebitedQuantity>
    <cbc:LineExtensionAmount currencyID="PEN">100.00</cbc:LineExtensionAmount>
    <cac:PricingReference>
      <cac:AlternativeConditionPrice>
        <cbc:PriceAmount currencyID="PEN">118.00</cbc:PriceAmount>
    ...
    <cac:Item>
      <cbc:Description><![CDATA[Intereses por mora en pago de factura F001-1]]></cbc:Description>
    </cac:Item>
    <cac:Price>
      <cbc:PriceAmount currencyID="PEN">100.00</cbc:PriceAmount>
    </cac:Price>
...
| 10 | Codigos de Tipo de Nota de Debito |
