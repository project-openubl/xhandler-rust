...
export type IEmailRecepientInputProps = {
  value: string[];
...
export const EmailRecepientInput: React.FC<IEmailRecepientInputProps> = ({
  value,
}) => {
  ...
  React.useEffect(() => {
    ...
    if (inputValue && inputValue.trim().length === 11) {
      newSelectOptions = [
        ...newSelectOptions,
  ...
  const createItemId = (value: any) =>
    `select-multi-create-typeahead-${value.replace(" ", "-")}`;
  ...
  return (
    ...
    >
      <SelectList
        ...
        id="select-multi-create-typeahead-listbox"
      >
